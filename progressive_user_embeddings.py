...
EXPERIMENTNAME = "user_embeddings_progressive"
...
POS_WEIGHTS = [min(np.float32(x),5) for x in POS_WEIGHTS]
...
def main():
    ...
    # Loading datasets
    # TODO: fix directories
    training_dataset = get_training_dataset(os.path.join(SOURCE_PATH, "GroundTruth/train_active_clipped.csv"))
    val_dataset = get_validation_dataset(os.path.join(SOURCE_PATH, "GroundTruth/validation_active_clipped.csv"))
    ...
    all_training_variables = tf.trainable_variables()
    embeddings_vars = [var for var in all_training_variables if 'embedding' in var.name or "flattened" in var.name or "Fully" in var.name]
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
        train_step_embeds = tf.train.AdadeltaOptimizer(learning_rate).minimize(loss, global_step=global_step,var_list=embeddings_vars) 

    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
        train_step_all = tf.train.AdadeltaOptimizer(learning_rate).minimize(loss, global_step=global_step,var_list=all_training_variables) 
