...
def get_model(user_embeddings, current_keep_prob, train_phase):
    ...
    with tf.name_scope('Fully_connected_2'):
        dropped = tf.nn.dropout(embeds_1_norm, keep_prob=current_keep_prob)
...
def main():
    ...
    logits, model_output = get_model(embeddings_input, current_keep_prob, train_phase)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for batch_counter in range(TRAINING_STEPS):
                ...
                summary, batch_loss[batch_counter], batch_accuracy[batch_counter], _ = sess.run(
                    ...
                    feed_dict={current_keep_prob: 0.3, y: batch_labels,
        ...
        for test_batch_counter in range(TEST_NUM_STEPS):
            ...
            test_pred_prob[start_idx:end_idx, :],test_one_hot[start_idx:end_idx, :] = sess.run([model_output, one_hot],
                                                            feed_dict={embeddings_input: test_embeddings,
                                                                       current_keep_prob: 1.0,
                                                                       train_phase: False})
